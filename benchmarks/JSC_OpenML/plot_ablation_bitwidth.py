...
def plot_bitwidth_vs_lut_ff(
    ...
    l1, = ax1.plot(
        bitwidth, lut, marker="o", label="LUT usage", color=lut_color,
        path_effects=[pe.Stroke(linewidth=1.9, foreground="white"), pe.Normal()],
        zorder=3,
    )
    ax1.set_xlabel("Model Bitwidth")
    ...
    l2, = ax2.plot(
        bitwidth, ff, marker="s", linestyle="--", label="FF Usage", color=ff_color,
        path_effects=[pe.Stroke(linewidth=1.9, foreground="white"), pe.Normal()],
        zorder=3,
    )
...
if __name__ == "__main__":
    ...
    LUT = [61297, 14403, 4025, 1496, 859, 338 ]
    FF = [2930, 2018, 1567, 1191, 720, 378]
   
    plot_bitwidth_vs_lut_ff(
        bitwidth=BITWIDTH,
        lut=LUT,
        ff=FF,
        single_column=True,   # False for a wider, two-column figure
        export_png=True,
        out_name="bitwidth_resources",
    )

...
def plot_layerwidth_vs_lut_ff(
    ...
    l1, = ax1.plot(
        layerwidth, lut, marker="o", label="LUT usage", color=lut_color,
        path_effects=[pe.Stroke(linewidth=1.9, foreground="white"), pe.Normal()],
        zorder=3,
    )
    ax1.set_xlabel("JSC OpenML Middle Layer Width")
    ...
    l2, = ax2.plot(
        layerwidth, ff, marker="s", linestyle="--", label="FF Usage", color=ff_color,
        path_effects=[pe.Stroke(linewidth=1.9, foreground="white"), pe.Normal()],
        zorder=3,
    )
...
if __name__ == "__main__":
    ...
    LUT = [158, 377, 821, 1629, 3044, 6210, 12455]
    FF = [178, 319, 640, 1284, 2417, 4941, 9464]
   
    plot_layerwidth_vs_lut_ff(
        layerwidth=LAYER_WIDTH,
        lut=LUT,
        ff=FF,
        single_column=True,   # False for a wider, two-column figure
        export_png=True,
        out_name="layerwidth_resources",
    )

# === Moons + MLP (same size as KAN_OG) + LBFGS wrapper ===
...
y_tr_t = torch.from_numpy(y_tr).float().to(device)              # [Ntr] float in {0.,1.}
y_te_t = torch.from_numpy(y_te).float().to(device)              # [Nte] float in {0.,1.}
...
# -------------------------------
# Model: MLP with same layer sizes [2, 8, 1]
# -------------------------------
class MLP(nn.Module):
    def __init__(self, sizes=(2, 8, 1), base_activation=nn.GELU):
        super().__init__()
...
def mlp_train_lbfgs(model, dataset, steps, loss_fn, lr, metrics):
    model.train()

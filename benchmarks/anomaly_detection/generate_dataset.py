...
def generate_train_AD_data():
    ...
    np.save(f"{data_dir}/processed/X_train.npy", res)
    np.save(f"{data_dir}/processed/y_train.npy", np.repeat([1 if "anomaly" in file.split("/")[-1] else 0 for file in files], samples_per_file))

def generate_test_AD_data():
    ...
    np.save(f"{data_dir}/processed/X_test.npy", np.array(data_array))
    np.save(f"{data_dir}/processed/y_test.npy", np.array(labels))

if __name__ == "__main__":
    generate_train_AD_data()
    generate_test_AD_data()

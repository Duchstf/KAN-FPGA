...
def apply_academic_style():
    """Applies a consistent plotting style for academic publications."""
...
# ---------- plotting ----------
def _plot_one_model(ax, runs, label, window=WINDOW, text_offset=(6, 0)):
    """
    Plots a single model's data, accepting a text_offset for the annotation.
    """
    if not runs:
        return
    ...
    # Annotate last value using the provided text_offset
...
def plot_models(
    ...
    # Define custom offsets for each model's text label (horizontal, vertical)
    # The order must match the 'models' list.
    text_offsets = [
        (6, 3),    # MLP FP: Push up
        (6, -3),   # MLP 8-bit: Push down
        (6, 0),    # KAN FP: Push down slightly
        (6, 0),     # KAN 8-bit: Push up slightly
        (6, 0),     # KAN 16-bit: Push up slightly
    ]

    for i, (path_like, label) in enumerate(models):
        runs = _load_runs_any(path_like)
        print(f"Found {len(runs)} runs for '{label}' in '{path_like}'")
        # Pass the corresponding offset to the plotting function
        offset = text_offsets[i] if i < len(text_offsets) else (6, 0)
        _plot_one_model(ax, runs, label=label, window=window, text_offset=offset)
...
if __name__ == "__main__":
    plot_models(
        models=[
            ...
            ("logs_kan_quant_16bit", "KAN 16-bit: [17, 6]"),
        ],
